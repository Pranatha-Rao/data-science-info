...
## [your code here]
##Code for obtaining data view from "df":
df4 <- filter(df, Grad.Rate>=80) ##Selecting columns with Grad.Rate>=80
df5 <- select(df4, X, Grad.Rate) ##Selecting the universities along with Grad.Rate column
df6 <- arrange(df5, desc(Grad.Rate)) ##Arranging them in descending order
df6
...
## [your code here]
##Code for finding max and min tuition grouped by private school in dataset df
dfy <- group_by(df, Private) ##Grouping by Private schools for dataset df
dfy <- summarize(dfy, max.tuition=max(Outstate), min.tuition=min(Outstate)) ##Finding max and min tuition 
dfy

##Code for finding max and min tuition grouped by private school in dataset college
dfx <- group_by(college, Private) ##Grouping by Private schools for dataset college
dfx <- summarize(dfx, max.tuition=max(Outstate), min.tuition=min(Outstate)) ##Finding max and min tuition
dfx
